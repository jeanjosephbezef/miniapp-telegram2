"""
Migration : ajoute la colonne 'annonce_texte' à la table 'parametre'
si elle n'existe pas déjà.

Usage :
    python migrate_annonce_texte.py
...
def ajouter_colonne_si_absente(cursor, table):
    if colonne_existe(cursor, table, "annonce_texte"):
        print(f"✔ '{table}' a déjà la colonne annonce_texte, rien à faire.")
        return

    cursor.execute(
        f"ALTER TABLE {table} ADD COLUMN annonce_texte VARCHAR(255)"
    )
    print(f"✔ Colonne annonce_texte ajoutée à '{table}'.")
...
def main():
    ...
    try:
        ajouter_colonne_si_absente(cursor, "parametre")
